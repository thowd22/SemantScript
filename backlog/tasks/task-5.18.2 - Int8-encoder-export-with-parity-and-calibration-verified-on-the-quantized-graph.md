...
2026-09-24 decision with the user: int8 dynamic quantization is dropped from the Phase 1 latency path (measured dead end for this model: 1.38x, one decision change per thousand, refused by the strict gate). The verified derivation gate stays as the mechanism for any future post-training graph change (depth-routed prefixes, distilled encoders). TASK-5.18 now depends on TASK-5.18.1 and TASK-6.7 instead.
<!-- SECTION:NOTES:END -->
