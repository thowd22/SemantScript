...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The compiler groups sema expressions into domains by controller, file or an explicit @domain tag and records the adapter per stage in the execution plan
- [x] #2 The trainer trains one adapter per domain on that domain's data, and the runtime applies the adapter named by the plan with no learned routing
- [x] #3 On an application with at least three domains, single-adapter and routed-adapter artifacts are compared on per-domain accuracy, pair-consistency, p50 latency and artifact size
- [x] #4 Interference is measured: adding or retraining one domain does not change the accuracy of the others beyond a stated tolerance
- [x] #5 A backlog decision records whether routed adapters become the default artifact layout
- [x] #6 The execution plan records a depth per domain alongside its adapter, and the runtime runs only that prefix of the shared encoder for the stages of that domain
- [x] #7 For the refund domain, attested final-set accuracy and CPU-runtime p50 latency are reported at three or more depths including the full stack
- [x] #8 A depth-routed refund artifact meets the Phase 1 latency criterion (p50 below 10 ms on the CPU Node runtime) with attested accuracy within one percentage point of the full-depth artifact, or the decision in criterion 5 records why not
<!-- AC:END -->
...
Typed-decisions routed comparison done (results-routed-2026-09-24): jointly trained over one encoder at full depth, single adapter 0.698 vs four routed adapters 0.659 over 2,000 decisions (two domains up, two down), same latency shape, +1.2 MB; interference run: three domains joint, agent_trace added on the frozen encoder in 117 s, the three unchanged exactly (delta 0.000, no weight changed), the added domain 0.364 vs 0.710 joint. Decision-10 recorded: routed adapters are the default layout for isolation and depth routing, not for accuracy; new or wholly changed domains retrain jointly.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Built compile-time routed domain adapters with depth routing end to end and measured them. Compiler: @domain headers, class and file default domains, --route-domains and --domain-depth, plan domains and per-stage adapter refs, model.encoderDepth. Trainer and model: one adapter per domain at its depth over the shared encoder, incremental adds on the frozen encoder, a v2 build cache storing the encoder and each adapter separately, routed export of one encoder prefix per depth. Runtime: per-function encoder and adapter selection with no learned routing. Verified by tests at every layer (Node 285, Python 444) and two experiments. Refund depth sweep: every depth passes the strict gate with zero attested misses; on the frozen final set through the CPU Node runtime depth 4 gives 160/160 at 7.9 ms p50 and depth 6 160/160 at 9.8 ms against 159/160 at 29.3 ms for the full stack, so the Phase 1 latency criterion is met by depth routing in the direct runtime measurement (harness rerun under TASK-5.18). Typed-decisions (four domains): routed adapters 0.659 vs single 0.698 at full depth, and adding a domain on the frozen encoder leaves the others byte-identical (delta zero) while the added domain reaches 0.364 vs 0.710 joint. Decision-10 adopts routed adapters as the default layout for isolation and depth routing, not accuracy.
