...
import { createBenchmarkResult, validatePredictionSet } from "../dist/index.js";
...
test("prediction/result schemas expose canonical task and role evidence boundaries", async () => {
  ...
  const baselineEvidence = structuredClone(
    makePredictionSet(dataset, "ollama-1b"),
  );
  ...
  for (const role of roles) {
    ...
    if (pin.model.revision !== null) {
      fixedIdentityMutations.push([
        "model",
        "revision",
        attackerDigest(pin.model.revision),
      ]);
    }
...
test("schemas retain SHA-shaped runtime identities for fields without publication pins", async () => {
  ...
  const semantscript = structuredClone(
    makePredictionSet(dataset, "semantscript"),
  );
  semantscript.system.model.revision = "e".repeat(64);
  semantscript.system.model.artifactSha256 = "f".repeat(64);
  rehash(semantscript);
  assert.equal(
    validatePredictions(semantscript),
    true,
    JSON.stringify(validatePredictions.errors),
  );

  const structuredApi = structuredClone(
  ...
  // The structured-output baseline may run through OpenRouter's Anthropic-format
  // route; the endpoint records it, and any other origin is rejected by the
  // schema and the runtime contract alike.
  const viaOpenRouter = structuredClone(structuredApi);
  viaOpenRouter.system.executionBackend.endpoint = "https://openrouter.ai/api";
  rehash(viaOpenRouter);
  assert.equal(
    validatePredictions(viaOpenRouter),
    true,
    JSON.stringify(validatePredictions.errors),
  );
  assert.equal(
    validatePredictionSet(viaOpenRouter).system.executionBackend.endpoint,
    "https://openrouter.ai/api",
  );
  const elsewhere = structuredClone(structuredApi);
  elsewhere.system.executionBackend.endpoint = "https://example.com";
  rehash(elsewhere);
  assert.equal(validatePredictions(elsewhere), false);
  assert.throws(() => validatePredictionSet(elsewhere), /endpoint/u);
});
...
test("schema cardinality ceilings match runtime contract ceilings", async () => {
  ...
  const ledgerSchema = await readSchema(
    "refund-training-input-ledger.v1.schema.json",
  );
  const predictionsSchema = await readSchema(
    "refund-benchmark-predictions.v1.schema.json",
  );
  const resultSchema = await readSchema(
    "refund-benchmark-result.v1.schema.json",
  );

  assert.equal(datasetSchema.properties.cases.maxItems, 20_000);
  assert.equal(
    common.$defs.humanAttestation.properties.caseIds.maxItems,
    20_000,
  );
  ...
  assert.equal(
    common.$defs.environmentProvenance.properties.runtimeVersions.maxItems,
    64,
  );
  assert.equal(
    common.$defs.measurementProtocol.properties.warmupIterations.maximum,
    10_000,
  );
  ...
  predictions.environment.runtimeVersions.push({
    name: "runtime-64",
    version: "test-only",
  });
