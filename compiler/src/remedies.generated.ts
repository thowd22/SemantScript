...
export const REMEDY_TEMPLATES = {
  ...
  "held-out-violation-example": {
    family: "verifier",
    fix: "add the examples entry {example}: constraint {index} ({source}) is broken on {count} of {size} held-out inputs (seed {seed}), and the constraints require that output for this one; or rerun with --cases {cases} (now {current}) so the region is sampled more densely",
    params: [
      "example",
      "index",
      "source",
  ...
  "held-out-violation-more-cases": {
    family: "verifier",
    fix: "rerun with --cases {cases} (now {current}): constraint {index} ({source}) is broken on {count} of {size} held-out inputs (seed {seed}), such as {inputs}; more cases cover the rule's region beyond its boundary, or record a tolerance with --max-constraint-violation-rate",
    params: [
      "cases",
      "current",
      "index",
