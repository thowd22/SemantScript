...
| `doctor-platforms`        | on `windows-latest` and `macos-latest`: `npm ci`, `npm run build`, a `.venv` with the CPU PyTorch build and `pip install -e '.[training]'`, then `node cli/bin/semantscript.js doctor --no-teacher` with the runner's bare Python (fails by design; the fix names the venv), again with the venv activated (must pass; the CPU `device` line is a warning), and `doctor --runtime`. The output is recorded in [environment.md](environment.md#recorded-runs); about 2.5 minutes in run 36166910601.                                                                                                                                                    | `node cli/bin/semantscript.js doctor` on that platform                                                                                    |
