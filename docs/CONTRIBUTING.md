...
| `cli/`                        | Node/TS   | `semantscript init \| doctor \| build \| train \| dev \| test \| run`.                                                                                                                                                      |
...
every pull request and on demand, as five parallel jobs on `ubuntu-latest`
(the `python` job runs twice, once per install) and a `doctor-platforms` job on
...
| `python` (`dev,training`) | the same, with the pinned `torch` installed first from the CPU wheel index (`--index-url https://download.pytorch.org/whl/cpu`) and then `pip install -e '.[dev,training]'`: every test that runs on a CPU, then `node cli/bin/semantscript.js doctor --python .venv/bin/python --no-teacher` (every Python-side check passes; the CPU `device` line is a warning; 7 s in run 36164465531). Only tests that need a live Ollama server, the ModernBERT CUDA smoke test, a network download of the pinned tokenizer, or a different install (the two environment-specific `test_primitives` cases) skip.                                                                                                                                                                                                                                                                                                                                                                                      | the same commands; a GPU is not used                                                                                                      |
| `fresh-install`           | the install exactly as the docs give it, with no npm cache: `npm install` and `npm run build` at the root, then `npm install`, `npm run build` and `npm test` in `examples/express-app` and in `examples/refund-service`, `npm run fixture-artifact` in the Express example, `npx --no-install semantscript --help`, `npx --no-install semantscript doctor --runtime` (the native bindings load after a first install) and the tutorial's `npx semantscript run` call there (fails if npm did not link the CLI bin), `docker build -f examples/express-app/deploy/Dockerfile .` and a smoke run of the image (`POST /tickets` and `POST /refunds/o1`).                                                                                                                                                                                                                                                                                                                                      | the same commands; `npm run fixture-artifact` refuses to replace an existing artifact unless run as `npm run fixture-artifact -- --force` |
| `package`                 | `npm ci`, `npm run build`, `.[training]` with CPU PyTorch into `.venv`, then in `examples/express-app` `npm install`, `npm run build` and `npm run fixture-artifact -- --quantizable` (the fixture encoder with a constant MatMul, plus fixture training datasets in `.semantscript/cache`), `npx --no-install semantscript releases derive --int8 --promote --python ../../.venv/bin/python` (the strict gate: no decision may change; `releases show` must name the int8 derivation), `npx --no-install semantscript package --include deploy/lambda.mjs --target lambda-zip` (exits 1 if the bundle exceeds Lambda's 250 MB unzipped limit) and a check that the bundle's manifest is `int8-dynamic`, one invocation of the packaged `deploy/lambda.mjs` handler with `node` from inside the bundle, `docker build -f deploy/Dockerfile.package .semantscript/package` (the bundle is the whole build context) and one `POST /tickets` against the running image, which prints its size. | the same commands ([Deploying](deploy.md))                                                                                                |
| `doctor-platforms`        | on `windows-latest` and `macos-latest`: `npm ci`, `npm run build`, a `.venv` with the CPU PyTorch build and `pip install -e '.[training]'`, then `node cli/bin/semantscript.js doctor --no-teacher` with the runner's bare Python (must exit 1 and name the venv in its fix), again with the venv activated (must pass; the CPU `device` line is a warning), and `doctor --runtime`; on Windows a `pwsh` step then runs doctor in PowerShell with the venv activated and `pytest trainer/tests/test_doctor.py -k windows_fix_lines`, which runs the Windows fix lines in real PowerShell and cmd. The output is recorded in [environment.md](environment.md#recorded-runs); about 2.5 minutes in run 36166910601.                                                                                                                                                                                                                                                                           | `node cli/bin/semantscript.js doctor` on that platform                                                                                    |
