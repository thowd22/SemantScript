...
| `package`                 | `npm ci`, `npm run build`, then in `examples/express-app` `npm install`, `npm run build` and `npm run fixture-artifact`, `npx --no-install semantscript package --include deploy/lambda.mjs --target lambda-zip` (exits 1 if the bundle exceeds Lambda's 250 MB unzipped limit), one invocation of the packaged `deploy/lambda.mjs` handler with `node` from inside the bundle, `docker build -f deploy/Dockerfile.package .semantscript/package` (the bundle is the whole build context) and one `POST /tickets` against the running image, which prints its size.                                                                                                                                               | the same commands ([Deploying](deploy.md))                                                                                                |
