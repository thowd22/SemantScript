...
looks like a release (the next prune deletes such leftovers). It refuses to run
when `current.json` is missing or invalid.

| Code                      | Cause                                                                                                                                                                                                      |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POINTER_INVALID`         | `current.json` is missing (for `prune` or an unnamed `rollback`), not a regular file, or not a v1 pointer whose release is `releases/sha256-<manifestSha256>`. A named `rollback` or `promote` repairs it. |
