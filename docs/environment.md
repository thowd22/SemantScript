...
| `teacher-config`   | The teacher file `train` would use exists and is a valid `[teacher]` table, or `--teacher constraints` names the built-in constraints teacher.                              | No file and no `ANTHROPIC_API_KEY`, or an invalid table: see [teachers](teachers.md); with complete constraints, `--teacher constraints` needs neither.               |
| `teacher-key`      | The key is in the environment (Ollama and the constraints teacher need none; with a `[teacher.fallback]` the fallback's key is checked); `ANTHROPIC_AUTH_TOKEN` counts too. | `export ANTHROPIC_API_KEY=…`; with an OpenRouter `base_url` and only `OPENROUTER_API_KEY` set, `export ANTHROPIC_API_KEY="$OPENROUTER_API_KEY"` (Windows: see below). |
| `teacher-probe`    | One minimal request succeeded; the line gives the latency and tokens. Skipped for the constraints teacher, which sends no request (its fallback, if any, is probed).        | A refused key, a wrong model name, or an Ollama server that is down or lacks the model (`ollama serve`, `ollama pull <model>`; a name without a tag is `:latest`).    |
...
semantscript doctor: ~/SemantScript/examples/express-app
  ...
  pass  teacher-config    /tmp/scratch/ollama.toml (ollama qwen2.5:1.5b-instruct-q4_K_M via 127.0.0.1:11434)
...
```text
  pass  teacher-config    /tmp/scratch/glm.toml (ollama glm-4.7-flash)
  pass  teacher-key       the Ollama backend needs no key
