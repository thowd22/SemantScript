...
| `model`              | Logical refs, never paths: `encoder`, `adapter` and `heads: [{outputPath, ref}]`, one head per scalar output (`outputPath` `""`) or per field (JSON Pointer `/field`, with `~0`/`~1` escaping).                                                                                                                                                           |
...
among functions with identical inputs.

## Application artifact manifest (`schemas/application-artifact.v1.schema.json`)

The runtime's contract: one tokenizer, one shared encoder, one adapter and one
or more heads per function, all verified. A runtime never needs source text,
examples, constraints, a teacher or the network.
...
| `model`              | `tokenizerRef`, `encoderRef`, `adapterRef`: the application-level defaults.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `functions[]`        | `id`, `semanticSha256`, the exact `inputs` descriptors plus `inputSchemaSha256` and `outputSchemaSha256`, `adapterRef`, `heads`, `runtime`, `verification`, `trainingProvenance`.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `trainingProvenance` | `datasetSha256`, `trainingKeySha256`, and the `teacher` and `baseModel` identity strings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

Model ABI v1: the encoder takes `input_ids` and `attention_mask` (`int64`,
`[BATCH, SEQUENCE]`) and emits `sentence_embedding` (`float32`, `[BATCH,
HIDDEN]`); the adapter maps it to `function_embedding` of the same shape; each
head maps that to `logits` shaped `[BATCH, 1]` for a boolean or `[BATCH, K]`
for `K` support values. The runtime divides logits by the head's temperature
