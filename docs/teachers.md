...
- **Time.** Requests times the seconds per request: the `[teacher.pricing]` figure, else the mean the last metered run of the same teacher recorded (`<cache-dir>/teacher-stats.json`), else a pinned figure (4 s for Sonnet 5 through OpenRouter; 1 s for Qwen3-14B through Ollama on one RX 9070 XT, both measured on the Express prompts). Requests sent as a Message Batch (the default Anthropic teacher, `mode = "auto"`, batches a synthetic dataset of at least `batch_threshold` cases) do not add per-request time: each batch adds one hour, since Anthropic's documentation says most batches finish within an hour, and the time column then shows a maximum as well, `(max …)`, counting the full `poll_timeout_seconds` (24 hours by default, the longest the run waits) for the first batch and, on a constrained expression, for each of the three label-replacement rounds. A line under the table says how many requests go through the batch.
