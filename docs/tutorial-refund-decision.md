...
| Sonnet 5 through OpenRouter              | `[teacher]` TOML with `backend = "anthropic"`, `model = "anthropic/claude-sonnet-5"`, `base_url = "https://openrouter.ai/api"`, `mode = "direct"`; `ANTHROPIC_API_KEY` set to the OpenRouter key | an OpenRouter key        | about USD 0.017 per request with the prompt of 2026-09-25 (the Express example's two expressions at 192 cases cost about USD 10 including adversarial cases); the compact, cached prompt now costs USD 0.0017 to 0.0071 per request, and `semantscript train --estimate` prints the figure for your run |
