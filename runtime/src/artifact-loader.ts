...
const PORTABLE_PATH = /^[A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)*(?:\/[A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)*)*$/u;
...
export class ArtifactLoadError extends Error {
  ...
  constructor(code: ArtifactLoadErrorCode, message: string, artifactPath?: string, cause?: unknown) {
...
export async function loadArtifact<PreparedResource = Uint8Array>(
  ...
  let aggregate = 0;
  const verified: { readonly metadata: ArtifactResourceV1; readonly bytes: Uint8Array }[] = [];

  for (const metadata of manifest.resources) {
    if (metadata.byteLength > settings.resourceBytes) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_QUOTA", `resource ${metadata.ref} exceeds quota`, metadata.path);
    }

    aggregate += metadata.byteLength;
    if (!Number.isSafeInteger(aggregate) || aggregate > settings.aggregateBytes) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_QUOTA", "aggregate resource quota exceeded");
    }
    ...
    if (bytes.byteLength !== metadata.byteLength || digest(bytes) !== metadata.sha256) {
      throw new ArtifactLoadError(
        "SEMA_ARTIFACT_INTEGRITY",
  ...
  try {
    for (const entry of verified) {
      const prepared = await settings.backend.prepareResource(entry.metadata, entry.bytes);
...
function resolveOptions<P>(options: ArtifactLoadOptions<P>): ResolvedOptions<P> {
  const runtimeVersion = options.runtimeVersion ?? "0.0.0";
  if (!SEMVER.test(runtimeVersion)) throw new TypeError("runtimeVersion must be valid semver");
  ...
  return {
    ...
    pointerBytes: positiveInteger(options.maximumPointerBytes ?? 1024 * 1024, "maximumPointerBytes"),
    manifestBytes: positiveInteger(options.maximumManifestBytes ?? 8 * 1024 * 1024, "maximumManifestBytes"),
    resourceBytes: positiveInteger(options.maximumResourceBytes ?? 1024 * 1024 * 1024, "maximumResourceBytes"),
...
async function selectRelease(artifactPath: string, pointerLimit: number): Promise<SelectedRelease> {
  if (artifactPath.length === 0) throw new TypeError("artifactPath must not be empty");
  const requested = resolve(artifactPath);
  const stats = await safeLstat(requested, "artifact directory");
  if (stats.isSymbolicLink() || !stats.isDirectory()) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", "artifact path must be a non-symlink directory", requested);
  }
  ...
  try {
    ...
    if (!isEnoent(error)) throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", "cannot inspect current.json", canonical, error);
  }

  if (hasPointer) {
    const pointerPath = join(canonical, "current.json");
    const bytes = await readSafeFile(canonical, "current.json", pointerLimit, "deployment pointer");
    ...
    if (!match || match[1] !== pointer.manifestSha256) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_INVALID_POINTER", "pointer digests differ", pointerPath);
    }
  ...
  if (!match?.[1]) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", "path is neither an artifact root nor sha256 release", canonical);
  }

  const releases = dirname(canonical);
  return {
    artifactRoot: basename(releases) === "releases" ? dirname(releases) : releases,
...
function parseJson(bytes: Uint8Array, limit: number, path: string): unknown {
  try {
    ...
    if (error instanceof StrictJsonError) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_INVALID_JSON", error.message, path, error);
    }
    throw error;
  }
}
...
function validatePointer(value: unknown): asserts value is ArtifactPointerV1 {
  ...
  exact(pointer, ["kind", "pointerVersion", "release", "manifestSha256"], "pointer", "SEMA_ARTIFACT_INVALID_POINTER");
  equal(get(pointer, "kind"), "semantscript.artifact-pointer", "pointer.kind", "SEMA_ARTIFACT_INVALID_POINTER");
  equal(get(pointer, "pointerVersion"), 1, "pointer.pointerVersion", "SEMA_ARTIFACT_INVALID_POINTER");
  matches(get(pointer, "release"), POINTER_RELEASE, "pointer.release", "SEMA_ARTIFACT_INVALID_POINTER");
  matches(get(pointer, "manifestSha256"), SHA256, "pointer.manifestSha256", "SEMA_ARTIFACT_INVALID_POINTER");
}

function validateManifest(value: unknown): asserts value is ApplicationArtifactManifestV1 {
  const manifest = object(value, "manifest");
  exact(manifest, ["kind", "artifactVersion", "irVersion", "compatibility", "application", "build", "resources", "model", "functions"], "manifest");
  equal(get(manifest, "kind"), "semantscript.application-artifact", "manifest.kind");
  equal(get(manifest, "artifactVersion"), 1, "manifest.artifactVersion");
  equal(get(manifest, "irVersion"), 1, "manifest.irVersion");

  const compatibility = object(get(manifest, "compatibility"), "manifest.compatibility");
  ...
  const application = object(get(manifest, "application"), "manifest.application");
  exact(application, ["id", "version"], "manifest.application");
  matches(get(application, "id"), /^[a-z][a-z0-9._-]{1,127}$/u, "manifest.application.id");
  matches(get(application, "version"), SEMVER, "manifest.application.version");

  const build = object(get(manifest, "build"), "manifest.build");
  exact(build, ["createdAt", "compilerVersion", "trainerVersion", "sourceIrSha256"], "manifest.build");
  const createdAt = text(get(build, "createdAt"), "manifest.build.createdAt");
  if (!isRfc3339DateTime(createdAt)) invalid("manifest.build.createdAt is not RFC 3339");
  matches(get(build, "compilerVersion"), SEMVER, "manifest.build.compilerVersion");
  matches(get(build, "trainerVersion"), SEMVER, "manifest.build.trainerVersion");
  matches(get(build, "sourceIrSha256"), SHA256, "manifest.build.sourceIrSha256");

  const resources = list(get(manifest, "resources"), "manifest.resources", 4);
  for (const [index, resource] of resources.entries()) validateResource(resource, `manifest.resources[${String(index)}]`);
  ...
  for (const [index, fn] of functions.entries()) validateFunction(fn, `manifest.functions[${String(index)}]`);
}

function validateResource(value: unknown, path: string): void {
  const resource = object(value, path);
  const role = choice(get(resource, "role"), ["tokenizer", "encoder", "adapter", "head"], `${path}.role`);
  const common = ["ref", "role", "format", "formatVersion", "path", "byteLength", "sha256"];
  if (role === "tokenizer") {
    exact(resource, [...common, "maximumSequenceLength"], path);
    equal(get(resource, "format"), "tokenizer-json", `${path}.format`);
    const maximumSequenceLength = integer(get(resource, "maximumSequenceLength"), `${path}.maximumSequenceLength`, 1);
    if (maximumSequenceLength > 8192) invalid(`${path}.maximumSequenceLength must be <= 8192`);
  } else {
  ...
  const resourcePath = matches(get(resource, "path"), PORTABLE_PATH, `${path}.path`);
...
function validateTensor(value: unknown, path: string): void {
  ...
  choice(get(tensor, "dtype"), ["int64", "float16", "float32"], `${path}.dtype`);
  const shape = list(get(tensor, "shape"), `${path}.shape`, 1);
  for (const [index, dimension] of shape.entries()) {
    if (typeof dimension === "number") integer(dimension, `${path}.shape[${String(index)}]`, 1);
    else matches(dimension, SYMBOLIC_DIMENSION, `${path}.shape[${String(index)}]`);
  }
}

function validateFunction(value: unknown, path: string): void {
  const fn = object(value, path);
  exact(fn, ["id", "semanticSha256", "inputs", "inputSchemaSha256", "outputSchemaSha256", "adapterRef", "heads", "runtime", "verification", "trainingProvenance"], path);
  matches(get(fn, "id"), FUNCTION_ID, `${path}.id`);
  for (const key of ["semanticSha256", "inputSchemaSha256", "outputSchemaSha256"]) matches(get(fn, key), SHA256, `${path}.${key}`);
  logicalRef(get(fn, "adapterRef"), `${path}.adapterRef`);
  const inputs = list(get(fn, "inputs"), `${path}.inputs`);
  const inputBudget = { nodes: 0 };
  for (const [index, input] of inputs.entries()) validateInput(input, `${path}.inputs[${String(index)}]`, inputBudget);
  const heads = list(get(fn, "heads"), `${path}.heads`, 1);
  for (const [index, head] of heads.entries()) validateHead(head, `${path}.heads[${String(index)}]`);
  validatePolicy(get(fn, "runtime"), `${path}.runtime`);
  validateVerification(get(fn, "verification"), `${path}.verification`);
  const provenance = object(get(fn, "trainingProvenance"), `${path}.trainingProvenance`);
  exact(provenance, ["datasetSha256", "trainingKeySha256", "teacher", "baseModel"], `${path}.trainingProvenance`);
  matches(get(provenance, "datasetSha256"), SHA256, `${path}.trainingProvenance.datasetSha256`);
  matches(get(provenance, "trainingKeySha256"), SHA256, `${path}.trainingProvenance.trainingKeySha256`);
  nonempty(get(provenance, "teacher"), `${path}.trainingProvenance.teacher`);
  nonempty(get(provenance, "baseModel"), `${path}.trainingProvenance.baseModel`);
}

function validateInput(value: unknown, path: string, budget: { nodes: number }): void {
...
function validateInputType(value: unknown, path: string, budget: { nodes: number }, depth: number): void {
  budget.nodes += 1;
  if (depth > 64 || budget.nodes > 100_000) invalid(`${path} exceeds input type complexity limits`);
  ...
  if (["string", "boolean", "number", "null"].includes(kind)) {
    ...
    const base = choice(get(type, "base"), ["string", "number"], `${path}.base`);
    ...
    exact(type, ["kind", "items"], path);
    for (const [index, entry] of list(get(type, "items"), `${path}.items`).entries()) validateInputType(entry, `${path}.items[${String(index)}]`, budget, depth + 1);
  } else if (kind === "object") {
    exact(type, ["kind", "name", "fields"], path);
    nonempty(get(type, "name"), `${path}.name`);
    const names = new Set<string>();
    for (const [index, entry] of list(get(type, "fields"), `${path}.fields`).entries()) {
      ...
      validateInputType(get(field, "type"), `${fieldPath}.type`, budget, depth + 1);
    ...
    for (const [index, variant] of variants.entries()) validateInputType(variant, `${path}.variants[${String(index)}]`, budget, depth + 1);
...
function validateHead(value: unknown, path: string): void {
  ...
  exact(head, ["outputPath", "headRef", "type", "parameterization", "calibration", "verification"], path);
  ...
  const parameterization = choice(get(head, "parameterization"), ["binary-sigmoid", "categorical-softmax"], `${path}.parameterization`);
  if ((kind === "boolean") !== (parameterization === "binary-sigmoid")) invalid(`${path}.parameterization does not match head type`);
  validateCalibration(get(head, "calibration"), `${path}.calibration`);
  const verification = object(get(head, "verification"), `${path}.verification`);
  exact(verification, ["accuracy", "pairConsistency"], `${path}.verification`);
  unit(get(verification, "accuracy"), `${path}.verification.accuracy`);
  unit(get(verification, "pairConsistency"), `${path}.verification.pairConsistency`);
}

function validateHeadType(value: unknown, path: string): string {
  const type = object(value, path);
  const kind = choice(get(type, "kind"), ["boolean", "nominal-string", "ordinal-string", "nominal-number", "ordinal-number"], `${path}.kind`);
  if (kind === "boolean") {
    ...
    for (const [index, entry] of support.entries()) text(entry, `${path}.support[${String(index)}]`);
    uniqueSemantic(support, `${path}.support`);
  } else if (kind === "nominal-number") {
    exact(type, ["kind", "support"], path);
    const support = list(get(type, "support"), `${path}.support`, 2);
    for (const [index, entry] of support.entries()) number(entry, `${path}.support[${String(index)}]`);
    uniqueSemantic(support, `${path}.support`);
  } else {
    exact(type, ["kind", "sourceKind", "minimum", "maximum", "step", "supportDecimal"], path);
    ...
    const support = list(get(type, "supportDecimal"), `${path}.supportDecimal`, 2);
    const supportDecimal = support.map((entry, index) =>
      canonicalDecimal(entry, `${path}.supportDecimal[${String(index)}]`),
    );
    validateOrdinalSupport(sourceKind, minimum, maximum, step, supportDecimal, path);
  }
  return kind;
}
...
function validateOrdinalSupport(
  ...
  const parsed = [minimumText, maximumText, stepText, ...supportText].map(parseDecimal);
  ...
  if (minimum === undefined || maximum === undefined || step === undefined || step <= 0n) {
    invalid(`${path} requires a positive ordinal step`);
  }
  if (sourceKind === "bounded-int") {
    if (stepText !== "1" || [minimumText, maximumText, ...supportText].some((value) => value.includes("."))) {
      invalid(`${path} bounded-int support requires integral values and step 1`);
    }
  }
  ...
  for (let index = 0; index < support.length; index += 1) {
    if (support[index] !== minimum + step * BigInt(index)) {
      invalid(`${path}.supportDecimal is not the exact ascending min/max/step sequence`);
    }
  }
  ...
  for (const [index, value] of supportText.entries()) {
    ...
    if (!Number.isFinite(converted)) {
      invalid(`${path}.supportDecimal[${String(index)}] does not convert to finite binary64`);
    }
    const key = binary64Key(converted);
    if (binary64.has(key)) {
      invalid(`${path}.supportDecimal values collide after binary64 conversion`);
    }
    binary64.add(key);
  }
}

function parseDecimal(value: string): { readonly integer: bigint; readonly scale: number } {
...
function validateCalibration(value: unknown, path: string): void {
  ...
  exact(calibration, ["method", "temperature", "ece", "brier", "sampleCount", "splitSha256", "eceBins"], path);
  equal(get(calibration, "method"), "temperature-scaling", `${path}.method`);
  if (number(get(calibration, "temperature"), `${path}.temperature`) <= 0) invalid(`${path}.temperature must be positive`);
...
function validatePolicy(value: unknown, path: string): void {
  ...
  exact(policy, ["resultMode", "confidenceThreshold", "policy", "fallbackRef"], path);
  choice(get(policy, "resultMode"), ["value", "diagnostic"], `${path}.resultMode`);
  const threshold = get(policy, "confidenceThreshold");
  if (threshold !== null) unit(threshold, `${path}.confidenceThreshold`);
  choice(get(policy, "policy"), ["none", "scalar-top1", "all-fields"], `${path}.policy`);
...
function validateCompatibility(manifest: ApplicationArtifactManifestV1, settings: ResolvedOptions<unknown>): void {
  if (compareSemver(settings.runtimeVersion, manifest.compatibility.minimumRuntimeVersion) < 0) {
    throw new ArtifactLoadError(
      "SEMA_ARTIFACT_INCOMPATIBLE",
  ...
  for (const capability of manifest.compatibility.requiredCapabilities) {
    if (!settings.capabilities.has(capability)) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_INCOMPATIBLE", `unsupported capability ${capability}`);
    }
  }
}

function validateRelationships(manifest: ApplicationArtifactManifestV1, opsets: ReadonlySet<number>): void {
  const resources = new Map<string, ArtifactResourceV1>();
  const paths = new Set<string>();
  for (const resource of manifest.resources) {
    if (resources.has(resource.ref)) invalid(`duplicate resource ref ${resource.ref}`);
    ...
    if (resource.role !== "tokenizer") {
      if (!opsets.has(resource.onnx.opset)) {
        throw new ArtifactLoadError("SEMA_ARTIFACT_INCOMPATIBLE", `unsupported ONNX opset ${String(resource.onnx.opset)}`, resource.path);
      }
  ...
  requireRole(resources, manifest.model.tokenizerRef, "tokenizer", "manifest.model.tokenizerRef");
  const encoder = requireOnnxRole(resources, manifest.model.encoderRef, "encoder", "manifest.model.encoderRef");
  requireRole(resources, manifest.model.adapterRef, "adapter", "manifest.model.adapterRef");
  ...
  for (const resource of manifest.resources) {
    if (!referenced.has(resource.ref)) {
      invalid(`resource ${resource.ref} is not selected by the model or any function`);
    }
  }
}
...
function validateFunctionRelationships(
  ...
  for (const [inputIndex, input] of fn.inputs.entries()) {
    if (input.index !== inputIndex) invalid(`${path}.inputs[${String(inputIndex)}].index must equal its position`);
    addUnique(names, input.name, `${path}.inputs`, "input name");
  }
  if (digest(semanticBytes(fn.inputs)) !== fn.inputSchemaSha256) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_INTEGRITY", `${path}.inputSchemaSha256 does not match inputs`);
  }

  const adapter = requireOnnxRole(resources, fn.adapterRef, "adapter", `${path}.adapterRef`);
  referenced.add(adapter.ref);
  adjacent(encoder.onnx.outputs[0], adapter.onnx.inputs[0], encoder.ref, adapter.ref);
  const outputPaths = new Set<string>();
  const scalarOutput = fn.heads.length === 1 && fn.heads[0]?.outputPath.length === 0;
  for (const [headIndex, binding] of fn.heads.entries()) {
    if (!scalarOutput && binding.outputPath.length !== 1) invalid(`${path}.heads must describe scalar or flat output`);
    addUnique(outputPaths, JSON.stringify(binding.outputPath), `${path}.heads`, "output path");
    const head = requireOnnxRole(resources, binding.headRef, "head", `${path}.heads[${String(headIndex)}].headRef`);
    referenced.add(head.ref);
    adjacent(adapter.onnx.outputs[0], head.onnx.inputs[0], adapter.ref, head.ref);
...
function validateRuntimePolicyRelationships(
  ...
  if (confidenceThreshold === null) {
    if (policy !== "none") {
      invalid(`${runtimePath}.policy must be none when confidenceThreshold is null`);
    }
    if (fallbackRef !== null) {
      invalid(`${runtimePath}.fallbackRef must be null when confidenceThreshold is null`);
    }
    return;
  }
...
function uniqueTensorNames(resource: OnnxResourceV1): void {
  ...
  for (const tensor of resource.onnx.inputs) addUnique(inputs, tensor.name, resource.ref, "input tensor");
  for (const tensor of resource.onnx.outputs) addUnique(outputs, tensor.name, resource.ref, "output tensor");
}

function validateAbi(resource: OnnxResourceV1): void {
  const { inputs, outputs } = resource.onnx;
  if (resource.role === "encoder") {
    if (inputs.length !== 2 || outputs.length !== 1) invalid(`${resource.ref} must have two inputs and one output`);
    requireTensor(findTensor(inputs, "input_ids", resource.ref), "int64", ["BATCH", "SEQUENCE"], resource.ref);
    requireTensor(findTensor(inputs, "attention_mask", resource.ref), "int64", ["BATCH", "SEQUENCE"], resource.ref);
    embedding(findTensor(outputs, "sentence_embedding", resource.ref), resource.ref);
  } else if (resource.role === "adapter") {
    if (inputs.length !== 1 || outputs.length !== 1) invalid(`${resource.ref} must have one input and one output`);
    embedding(findTensor(inputs, "sentence_embedding", resource.ref), resource.ref);
    embedding(findTensor(outputs, "function_embedding", resource.ref), resource.ref);
  } else {
    if (inputs.length !== 1 || outputs.length !== 1) invalid(`${resource.ref} must have one input and one output`);
    embedding(findTensor(inputs, "function_embedding", resource.ref), resource.ref);
    const logits = findTensor(outputs, "logits", resource.ref);
    if (logits.dtype !== "float32" || logits.shape.length !== 2 || !batch(logits.shape[0])) {
...
function validateLogits(binding: HeadBindingV1, resource: OnnxResourceV1, path: string): void {
  const logits = resource.onnx.outputs[0];
  if (!logits) invalid(`${resource.ref} has no logits output`);
  const count = binding.type.kind === "boolean"
    ? 1
    : binding.type.kind === "ordinal-number"
      ? binding.type.supportDecimal.length
      : binding.type.support.length;
  if (logits.shape[1] !== count) invalid(`${path} support requires ${String(count)} logits`);
}
...
function requireRole(
  ...
  if (resource.role !== role) invalid(`${path} expected ${role}, found ${resource.role}`);
  return resource;
}
...
function findTensor(tensors: readonly TensorDescriptorV1[], name: string, ref: string): TensorDescriptorV1 {
...
function embedding(tensor: TensorDescriptorV1, ref: string): void {
  ...
  if (
    ...
    (hidden !== "HIDDEN" && (typeof hidden !== "number" || !Number.isSafeInteger(hidden) || hidden < 1))
  ) {
...
function requireTensor(
  ...
  if (tensor.dtype !== dtype || !sameShape(tensor.shape, shape)) invalid(`${path}.${tensor.name} has invalid ABI`);
}
...
async function safeDirectory(root: string, relativePath: string): Promise<string> {
  ...
  for (const segment of relativePath.split("/")) {
    ...
    if (stats.isSymbolicLink() || !stats.isDirectory()) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", "release contains a symlink or non-directory", current);
    }
  }
...
async function readSafeFile(
  ...
  if (!filename) throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} has no filename`, relativePath);
  let parent = root;
  for (const segment of segments) {
    parent = join(parent, segment);
    const stats = await safeLstat(parent, description);
    if (stats.isSymbolicLink() || !stats.isDirectory()) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} has an unsafe parent`, parent);
    }
  }
  ...
  if (pathStats.isSymbolicLink() || !pathStats.isFile()) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} is not a non-symlink regular file`, target);
  }

  const handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW).catch((error: unknown) => {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `cannot safely open ${description}`, target, error);
  });
  try {
    const before = await handle.stat();
    if (!before.isFile()) throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} is not regular`, target);
    if (!Number.isSafeInteger(before.size) || before.size > maximumBytes) {
      throw new ArtifactLoadError("SEMA_ARTIFACT_QUOTA", `${description} exceeds quota`, target);
    }
    ...
    if (
      ...
      throw new ArtifactLoadError("SEMA_ARTIFACT_INTEGRITY", `${description} changed while loading`, target);
    }
...
async function safeLstat(path: string, description: string) {
  try {
    ...
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `cannot inspect ${description}`, path, error);
  }
}
...
function portablePath(path: string, description: string): void {
  if (path.length === 0 || path.length > 500 || isAbsolute(path) || !PORTABLE_PATH.test(path) || path.includes("\\")) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} is not a portable relative path`, path);
  }
}
...
function contained(root: string, target: string, description: string): void {
  ...
  if (child === ".." || child.startsWith(`..${sep}`) || isAbsolute(child)) {
    throw new ArtifactLoadError("SEMA_ARTIFACT_PATH", `${description} resolves outside its root`, target);
  }
}
...
function exact(
  ...
  for (const key of Object.keys(value)) {
    if (!expected.has(key)) throw new ArtifactLoadError(code, `${path} has unsupported property ${key}`);
  }
  for (const key of keys) {
    if (!Object.prototype.hasOwnProperty.call(value, key)) throw new ArtifactLoadError(code, `${path} is missing ${key}`);
  }
}

function list(value: unknown, path: string, minimum = 0, maximum = Number.MAX_SAFE_INTEGER): unknown[] {
  if (!Array.isArray(value) || value.length < minimum || value.length > maximum) {
    invalid(`${path} must be an array with ${String(minimum)} to ${String(maximum)} items`);
  }
  return value;
}
...
function matches(
  ...
  if (typeof value !== "string" || !pattern.test(value)) throw new ArtifactLoadError(code, `${path} is invalid`);
  return value;
}
...
function equal(
  ...
  if (value !== expected) throw new ArtifactLoadError(code, `${path} must equal ${JSON.stringify(expected)}`);
}
...
function uniqueSemantic(values: readonly unknown[], path: string): void {
  ...
  for (const value of values) addUnique(seen, hex(semanticBytes(value)), path, "semantic value");
}
...
function strictSemanticOrder(values: readonly unknown[], path: string): void {
  ...
  for (const value of values) {
    ...
    if (previous && compareBytes(previous, current) >= 0) invalid(`${path} is not in strict semantic byte order`);
    previous = current;
  }
}

function addUnique(set: Set<string>, value: string, path: string, description: string): void {
  if (set.has(value)) invalid(`${path} contains duplicate ${description} ${value}`);
  set.add(value);
}
...
function semanticBytes(value: unknown): Uint8Array {
  return new TextEncoder().encode(JSON.stringify(["semantscript-semantic-json", 1, typedNode(value)]));
}
...
function compareSemver(left: string, right: string): number {
  ...
  if (!leftMatch || !rightMatch) throw new TypeError("invalid semver comparison");
...
function sameShape(left: readonly (number | string)[], right: readonly (number | string)[]): boolean {
  return (
    left.length === right.length &&
    left.every((value, index) => {
      const expected = right[index];
      return expected === "BATCH" ? value === "BATCH" || value === 1 : value === expected;
    })
  );
}
...
function compatibleEdgeShape(
  ...
  return (
    ...
    left.every((value, index) => {
      ...
      if (index === 0 && (value === "BATCH" || value === 1) && (other === "BATCH" || other === 1)) {
        return true;
      }
      if (
