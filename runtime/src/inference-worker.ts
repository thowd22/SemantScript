...
function evaluateHead(
  ...
  return {
    ...
    diagnostic: {
      value: selected,
      confidence,
      uncertainty: normalizedEntropy(probabilities, invalidResult),
      distribution,
      expectedValue: expectedValue(head, probabilities, invalidResult),
    },
  };
}
...
function invalidResult(message: string): Error {
  return new WorkerInvocationError(INFERENCE_ERROR.invalidResult, message);
}
